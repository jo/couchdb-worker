...
function AttachmentsWorker(config, db) {
  ...
  function checkAttachment(doc, name) {
    var status = doc.worker_status
      && doc.worker_status[this.name]
      && doc.worker_status[this.name].attachments
      && doc.worker_status[this.name].attachments[name];
  ...
  config.processor = {
    ...
    process: function(doc, commit) {
      ...
      process.call(this, doc, attachment, function(error, attributes) {
        ...
        commit(error, attributes, { attachments: status });
      });
    }
  };
