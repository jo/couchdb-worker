 ...
 * CouchDB Worker Pool
 * 
 * Create a Worker for each database in _all_docs.
 *
...
WorkerPool.prototype._listen = function(db) {
  // ignore preserved databases like _users and _replicator
  if (db.match(/^_/)) return;

  this.workers.push(new this.klass(this.config, db));
};
