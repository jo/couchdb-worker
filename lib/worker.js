...
Worker.prototype._process = function(doc) {
  ...
  request({
    ...
    this.processor.process.call(this, doc, function(error, attributes, status) {
      ...
      this._commit(doc, attributes, error ? 'error' : 'completed', error, status);
    }.bind(this));
  }.bind(this));
};

// insert worker status into doc
Worker.prototype._setWorkerStatus = function(doc, status, error, additionalStatus) {
  ...
  // add additional status if present
  if (additionalStatus) extend(doc.worker_status[this.name], additionalStatus);
};

// TODO: implement resolveConflict
Worker.prototype._commit = function(doc, attributes, status, processError, additionalStatus) {
  this._setWorkerStatus(doc, status, processError, additionalStatus);
